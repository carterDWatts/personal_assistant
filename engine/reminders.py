...
class Reminders:
    ...
    async def save(self, args):
        ...
        with self.map.conn.transaction():
            if args.get('id'):
                result = self.map.row("update memory.reminders set title=%s,context=%s,timing=%s,window_start=%s,window_end=%s,next_notify_at=%s,followup_hours=%s,timezone=%s,version=version+1,updated_at=now() where id=%s and version=%s and status='open' returning *",
                    (args['title'],args['context'],args['timing'],start,end,first,hours,zone,identifier,args.get('version')))
                if not result: raise ValueError('Reminder changed. Read it again before editing.')
                return result
            self.map.execute("insert into memory.reminders(id,title,context,timing,window_start,window_end,next_notify_at,followup_hours,timezone,source_message_id) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) on conflict(id) do nothing",
                (identifier,args['title'],args['context'],args['timing'],start,end,first,hours,zone,self.tools.message_id))
