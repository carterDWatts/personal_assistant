...
The app connects automatically when opened and remembers your selected model and memory environment. Choosing Claude or ChatGPT, or switching test memory, reconnects automatically. Both models use the same shared transcript and knowledge map. Replies use the current conversation immediately; a smaller model saves structured memory in the background, with its progress shown separately. Talk starts a live voice conversation. The microphone stays on during replies; speaking interrupts playback and generation. End stops voice mode. Allow Microphone when macOS asks. Speech is transcribed locally with sherpa-onnx; it never goes to Apple's speech service. The build downloads pinned recognition and Kokoro voice models once (about 450 MB). Your words appear in a fixed live-transcription panel as you speak. Kokoro generates a British English voice locally, played through an echo-cancelled audio engine. A short pause submits each utterance. Actual interruption timing depends on speech recognition and your audio devices. Local unsigned development builds may require renewed macOS permission after rebuilding.
