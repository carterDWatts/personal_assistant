...
struct ChatMessage: Identifiable {
    ...
    static func stored(_ row: [String: Any]) -> ChatMessage? {
        ...
        return ChatMessage(role: role, text: content, databaseID: row["id"].map { String(describing: $0) }, reference: payload?["reference"] as? [String: String])
    }
}
...
struct ReplyState {
    ...
    private func finishRows(_ messages: inout [ChatMessage]) {
        messages.removeAll { $0.role == "assistant" && $0.text.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty }
