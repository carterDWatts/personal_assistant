...
@main struct PomodoroCheck {
    static func main() throws {
        ...
        timer.start(at: now)
        precondition(timer.blocks(at: now) == 1)
        precondition(timer.blocks(at: now.addingTimeInterval(1)) == 1)
        precondition(timer.blocks(at: now.addingTimeInterval(187)) == 1)
        precondition(timer.blocks(at: now.addingTimeInterval(188)) == 2)
        precondition(timer.blocks(at: now.addingTimeInterval(1500)) == 8)
        precondition(timer.seconds(at: now.addingTimeInterval(100)) == 1400)
        timer.pause(at: now.addingTimeInterval(100))
        precondition(timer.blocks(at: now.addingTimeInterval(1000)) == 1)
