-- Make assistant-initiated status/completion messages visible on app launch/reconnect.
-- Existing outbound.post emits an inbox_received event when the worker creates the
-- message, but a phone that was offline/backgrounded needs the durable inbox in
-- bootstrap too.
...
declare result jsonb; history jsonb; inbox jsonb; source memory.messages; selected memory.messages; segment uuid; request uuid; source_id bigint; selected_id bigint;
begin
